...
public class ChiDoanController {
	...
	@GetMapping("/list")
	public String listChiDoan(Model model) {
    	model.addAttribute("chidoans", chidoanRepository.findAll());
    	return "chidoan/list";
    }
	
	@GetMapping("/create")
    public String registerForm(Model model) {
        model.addAttribute("chidoan", new Khoa());
	...
	@ResponseBody
	public ResponseEntity<Map<String, Object>> saveAddForm(@ModelAttribute ChiDoan chidoan, Model model) {
		Map<String, Object> response = new HashMap<>();
		try {
			chidoan.setMacd(taoMaKhoa(chidoan.getDonvi()));
			chidoanRepository.save(chidoan);
			response.put("code", 200);
            response.put("message", "Thêm mới thành công!");
            response.put("redirect", "/chidoan/list");
	...
	public String search(@ModelAttribute("keyword") String keyword, Model model) {
	    if (keyword == null || keyword.trim().isEmpty()) {
	        ...
	        model.addAttribute("chidoans", 
	        		chidoanRepository.findByDonviContainingIgnoreCaseOrMacdContainingIgnoreCase(keyword, keyword));
	    }
